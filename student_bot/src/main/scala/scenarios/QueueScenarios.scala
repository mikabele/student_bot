...
object QueueScenarios {
  def of[F[_]: TelegramClient: Monad: Concurrent](
    redisCommands: RedisCommands[F, String, String],
    queueService:  QueueService[F],
    authService:   AuthService[F]
  ): F[QueueScenarios[F]] = {
    val keys = Seq("addToQueue")
    for {
      values    <- keys.traverse(_ => Semaphore[F](0))
      semaphores = Map.from(keys.zip(values))
    } yield new QueueScenariosImpl[F](redisCommands, queueService, authService, semaphores)
  }
}
