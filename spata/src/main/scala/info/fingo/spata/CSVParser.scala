...
final class CSVParser[F[_]: Sync: Logger](config: CSVConfig) {
  ...
  /** Provides access to asynchronous parsing method.
    ...
    */
  def async(implicit F: Async[F]): CSVParser.Async[F] = new CSVParser.Async(this)
}
...
object CSVParser {
  ...
  /** [[CSVParser]] helper with asynchronous parsing method.
    ...
    */
  final class Async[F[_]: cats.effect.Async: Logger] private[CSVParser] (parser: CSVParser[F]) {
