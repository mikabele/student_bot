...
/** Type which represents a description of sequence of elements.
  ...
  *
  * `models.Episode` forms a monad in `O` with `pure` and `flatMap`.
  *
...
object Episode {
  ...
  final case class Pure[F[_], A](a: A) extends Episode[F, Any, A]

  final case class Next[F[_], A](p: A => Boolean) extends Episode[F, A, A]

  final case class Eval[F[_], A](fa: F[A]) extends Episode[F, Any, A]

  final case class RaiseError[F[_]](e: Throwable) extends Episode[F, Any, Nothing]

  final case class Protected[F[_], I, O1, O2 >: O1](
    episode: Episode[F, I, O1],
    onError: Throwable => Episode[F, I, O2]
  ) extends Episode[F, I, O2]

  final case class Bind[F[_], I, O1, O2](episode: Episode[F, I, O1], fn: O1 => Episode[F, I, O2])
    extends Episode[F, I, O2]

  final case class Cancellable[F[_], I, O](
    episode:   Episode[F, I, O],
    cancelOn:  I => Boolean,
    finalizer: Option[I => F[Unit]]
  ) extends Episode[F, I, O]

  final case class Tolerate[F[_], I, O](episode: Episode[F, I, O], limit: Option[Int], fn: I => F[Unit])
    extends Episode[F, I, O]

  final case class TimeLimited[F[_], I, O](episode: Episode[F, I, O], duration: FiniteDuration) extends Episode[F, I, O]
    ...
    episode match {
      ...
      case Protected(episode, onError) =>
        open(episode, input, cancelTokens, first).flatMap {
          ...
          case other: (Result[I, O], Stream[F, I]) => Stream(other)
        }
      ...
      case Bind(prev, fn: (Any => Episode[F, I, O])) =>
        Stream(prev)
  ...
  private def translate[F[_], G[_], I, O](episode: Episode[F, I, O], f: F ~> G): Episode[G, I, O] =
    episode match {
      ...
      case Protected(ep, onError)     => Protected(ep.mapK(f), onError.andThen(_.mapK(f))).asInstanceOf[Episode[G, I, O]]
